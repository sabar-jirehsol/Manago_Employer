...
class OfferLetterListServices {

  static BuildContext? context;

  static Future<OfferExperienceLetterListModel?> offerLetterListService(
    ...
    if (connectivityResult == ConnectivityResult.mobile ||
      ...
      try {
        ...
        print(api);
        print("offer letter  list ${response.body}");
        if (response.statusCode == 200) {
          OfferExperienceLetterListModel jsonResponse =
  ...
  static Future<ExperienceLetterListModel?> experienceLetterListService(
      String api,
      String id,
      String token,
    ...
    if (connectivityResult == ConnectivityResult.mobile ||
      ...
      try {
        ...
        print(response.body);
        if (response.statusCode == 200) {
          ExperienceLetterListModel jsonResponse =
              experienceLetterListModelFromJson(response.body);
          return jsonResponse;
        } else if (response.statusCode == 401) {
          final errorMessage = jsonDecode(response.body);
          Alert.showSnackbar(errorMessage['errorMessage'], context!);
