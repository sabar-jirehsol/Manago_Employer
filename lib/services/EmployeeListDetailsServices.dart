...
class EmployeeListDetailsServices {
  static BuildContext? context;
  static Future<EmployeeListDetailsModel?> getEmployeeListDetails(
    ...
    if (connectivityResult == ConnectivityResult.mobile ||
      ...
      try {
        ...
        // };
        print("LIST OF EMPLIYEE DETAILS ApI ${api}");
        final response = await http.get(Uri.parse(api),);
        print(response.statusCode);
        print(response.body);

        if (response.statusCode == 200) {
          EmployeeListDetailsModel jsonResponse =
              employeeListDetailsModelFromJson(response.body);
          return jsonResponse;
